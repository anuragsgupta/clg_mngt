package raven.javaswingdev.chart;
...
public class PieChart extends JComponent {
    ...
    private void drawPopupLabel(Graphics2D g2, double size, double angle, double labelX, double labelY, String text, String detail) {
        float fontSize = (float) (getFont().getSize() * size * 0.0039f);
        boolean up = !(angle > 0 && angle < 180);
        double space = size * 0.05f;
        double spaceV = size * 0.03f;
        double paceH = size * 0.03f;
        ...
        double width = Math.max(r1.getWidth() + paceH * 2, r2.getWidth() + paceH * 4);
        double height = r1.getHeight() + r2.getHeight() + spaceV * 4;
